...
function AnalyticsContent() {
  ...
  const conversionRate =
    totalCalls > 0 ? ((appointmentsBooked / totalCalls) * 100).toFixed(1) : "0";
  const missedRate =
    totalCalls > 0 ? ((missedCalls / totalCalls) * 100).toFixed(1) : "0";
  const upsellRate =
    upsellAttempts > 0
          ...
          <Card key={metric.title}>
            <CardContent className="pt-5 pb-5">
              <div className="flex items-start gap-3">
                <div
                  className={`w-9 h-9 rounded-lg ${metric.bg} flex items-center justify-center shrink-0`}
                >
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              ...
              Plan — {subscription.includedMinutes} minutes included
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="flex items-center justify-between text-sm">
              <span className="font-mono">
                {subscription.usedMinutes} minutes used
              </span>
              <span className="font-mono">
                {subscription.includedMinutes} minutes total
              </span>
            </div>
            <Progress value={subscription.usagePercent} className="h-2" />
            {subscription.overageMinutes > 0 && (
              <p className="text-sm text-destructive">
                {subscription.overageMinutes} overage minutes — $
                {subscription.overageCharge} additional charge
              </p>
            )}
          </CardContent>
        </Card>
      ...
      <Card className="border-dashed">
        <CardContent className="py-12 text-center">
          <p className="text-muted-foreground text-sm">
            Detailed charts and trend visualizations will be added in the design
            pass.
          </p>
        </CardContent>
      </Card>
