...
describe("buildRingShopFirstTwiML", () => {
  it("produces valid TwiML with the shop phone wrapped in <Number>", () => {
    const xml = buildRingShopFirstTwiML("+15551234567", 12);
  ...
  it("includes the timeout attribute on the <Dial> element", () => {
    const xml = buildRingShopFirstTwiML("+15551234567", 15);
    expect(xml).toContain('timeout="15"');
  });

  it("uses /api/twilio/no-answer as the action URL", () => {
    const xml = buildRingShopFirstTwiML("+15551234567", 12);
    expect(xml).toContain('action="/api/twilio/no-answer"');
  });
  ...
  it("supports a range of timeouts", () => {
    expect(buildRingShopFirstTwiML("+15551234567", 5)).toContain(
      'timeout="5"'
    );
    expect(buildRingShopFirstTwiML("+15551234567", 30)).toContain(
      'timeout="30"'
    );
  });
  ...
  it("escapes XML special characters in the shop phone number", () => {
    ...
    const xml = buildRingShopFirstTwiML('555 "evil" & <bad>', 12);
  ...
  it("handles E.164 numbers without modification", () => {
    const xml = buildRingShopFirstTwiML("+18005551234", 10);
    expect(xml).toContain("<Number>+18005551234</Number>");
  });

  it("handles US local format numbers", () => {
    const xml = buildRingShopFirstTwiML("(555) 123-4567", 12);
